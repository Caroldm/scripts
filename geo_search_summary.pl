...
# Get date (today)
my ($year1, $month1, $day1) = (localtime)[5,4,3];   
$a = (($year1+1900)."/"."$month1". "/".($day1)."");
#print($a);


# Get date - 7days
$b = (($year1+1900)."/"."$month1". "/".($day1-7)."");  
#print($b);
#print ("'$a'[PDAT] : '$b'[PDAT]" );
...
#Join accessions with OR
#my $query = join('+OR+',@acc_array);


# URLs
# Assemble esearch URL using the desired time period, type of organism,etc.
my $base = 'http://eutils.ncbi.nlm.nih.gov/entrez/eutils/';
my $url = $base . 'esearch.fcgi?db=gds&term=$query+AND+homo sapiens[ORGN]+OR+Mus musculus[ORGN]+OR+Rattus norvegicus[ORGN]+AND+"2015/11/30"[PDAT] : "2015/12/7"[PDAT]&usehistory=y';
...
# For ESearch-Esummary use - Include this code 
